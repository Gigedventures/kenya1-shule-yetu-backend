...
class ParentPortalController extends Controller
{
    ...
    { $this->authorizePermission('parent.view'); return response()->json($this->dashboard->build($parent)); }

    public function switch(string $parent, string $child): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->dashboard->switchChild($parent, $child)); }

    // ---- LIVE STATUS (8 features) ----
    public function liveStatus(string $student): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->presence->getStatus($student)); }

    public function busLocation(string $bus): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->bus->getBusLocation($bus)); }

    public function confirmPickup(Request $r): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->bus->confirmPickup($r->input('student'))); }

    public function confirmDropoff(Request $r): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->bus->confirmDropoff($r->input('student'))); }

    public function routeHistory(string $student): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->bus->getRouteHistory($student)); }

    // ---- TRIPS (5 features) ----
    public function trip(string $trip): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->trips->getTrip($trip)); }

    public function tripPhotos(string $trip): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->trips->getPhotos($trip)); }

    // ---- ACADEMICS (5 features) ----
    public function insights(string $student): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->academics->getInsights($student)); }

    // ---- FINANCE (4 features) ----
    public function statement(string $student): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->finance->getStatement($student)); }

    public function paymentHistory(string $student): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->finance->getHistory($student)); }

    // ---- COMMUNICATION (4 features) ----
    public function messages(string $parent): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->comm->getThreads($parent)); }

    public function sendMessage(Request $r): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->comm->send($r->input('parent'), $r->input('teacher'), $r->input('message'))); }

    // ---- WELLBEING (2 features) ----
    public function wellness(string $student): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->wellbeing->getDashboard($student)); }

    // ---- ATTENDANCE (4 features) ----
    public function attendanceSummary(string $student): JsonResponse
    { $this->authorizePermission('parent.view'); return response()->json($this->attendance->getSummary($student)); }
